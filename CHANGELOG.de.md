...
Alle nennenswerten Änderungen an diesem Projekt werden hier dokumentiert. ([🇬🇧 English version](CHANGELOG.md))
...
- Internationalisierung (i18n): die komplette Oberfläche ist jetzt auf Deutsch und Englisch
  verfügbar, umschaltbar über ein neues Sprachmenü (Globus-Icon) im Menüband mit
  Länderflaggen (Deutschland / UK).
- Englische Variante des Logos, die bei englischer Sprache automatisch angezeigt wird.
- Englische Vorlage für das Beispielprojekt (`defaultTemplates.en.ts`); Datei-Menü → Beispiel
  wählt je nach eingestellter Sprache die passende Vorlage.
- "Speichern unter…" (Datei-Menü sowie Strg+Umschalt+S).
- Die gewählte Sprache wird jetzt in `config.json` gespeichert und beim nächsten Start
  automatisch wiederhergestellt; der Fenstertitel für ein neues, ungespeichertes Projekt
  ("Neues Projekt" / "New Project") folgt ebenfalls der gewählten Sprache.

### Changed

- `recent.json` in `config.json` umbenannt und um das Feld für die gespeicherte Sprache
  erweitert.
