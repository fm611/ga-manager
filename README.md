...
| `GroupAddress.Web` | React 19 + TypeScript + Vite + Fluent UI. Enthält das komplette Domänenmodell, die Undo/Redo-Logik sowie CSV-Import/-Export. Kommuniziert mit dem WPF-Host über eine schmale `postMessage`-Bridge ([`src/host/wpfBridge.ts`](GroupAddress.Web/src/host/wpfBridge.ts)). |
...
dotnet build GroupAddress.UI.WPF/GroupAddress.UI.WPF.csproj
dotnet run --project GroupAddress.UI.WPF
```
...
```bash
dotnet publish GroupAddress.UI.WPF/GroupAddress.UI.WPF.csproj -c Release -r win-x64 --self-contained true -p:PublishProfile=FolderProfile
```

Ergebnis liegt unter `GroupAddress.UI.WPF/bin/Release/net10.0-windows7.0/publish/win-x64/`.
