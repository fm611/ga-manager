...
namespace GroupAddress.TestConsole
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var lightCommonMain_Switch_MiddleGroup = new SubGroup(1, "Schalten", lightCommonMainGroup);
            var lightCommonMain_SwitchStatus_MiddleGroup = new SubGroup(2, "Schalten Status", lightCommonMainGroup);
            var lightCommonMain_Lock_MiddleGroup = new SubGroup(3, "Sperren", lightCommonMainGroup);
            var lightCommonMain_LockStatus_MiddleGroup = new SubGroup(4, "Sperren Status", lightCommonMainGroup);

            var lightDimmMainGroup = new MainGroup(2, "Licht dimmbar");
            var lightDimm_BrightnessRel_MiddleGroup = new SubGroup(1, "Bright relativ", lightDimmMainGroup);
            var lightDimm_BrigthnessAbs_MiddleGroup = new SubGroup(2, "Bright absolut", lightDimmMainGroup);
            var lightDimm_BrightnesseValue_MiddleGroup = new SubGroup(3, "Bright Wert", lightDimmMainGroup);

            var lightTWMainGroup = new MainGroup(3, "Licht TW");
            var lightTW_TempRel_MiddleGroup = new SubGroup(1, "Temp relativ", lightTWMainGroup);
            var lightTW_TempAbs_MiddleGroup = new SubGroup(2, "Temp absolut", lightTWMainGroup);
            var lightTW_TempValue_MiddleGroup = new SubGroup(3, "Temp Wert", lightTWMainGroup);

            var lightRGBWMainGroup = new MainGroup(4, "Licht RGBW");
            var lightRGBW_Switch_MiddleGroup = new SubGroup(1, "Schalten", lightRGBWMainGroup);
            var lightRGBW_SwitchStatus_MiddleGroup = new SubGroup(2, "Schalten Status", lightRGBWMainGroup);
            var lightRGBW_DimmRel_MiddleGroup = new SubGroup(3, "Dimm relativ", lightRGBWMainGroup);
            var lightRGBW_DimmAbs_MiddleGroup = new SubGroup(4, "Dimm absolut", lightRGBWMainGroup);
            var lightRGBW_DimmValue_MiddleGroup = new SubGroup(5, "Dimm Wert", lightRGBWMainGroup);


            var light = ItemTemplate.Light.GetItem(lightCommonMainGroup, "EG_KU_Esstisch");
            ...
            //var gas = rgbwTemplate.CreateGA("EG_WZ_Spots");
